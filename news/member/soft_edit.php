    ...
    if(isset($addRow['softlinks']) && $addRow['softlinks']!='')
    {
        ...
        if(is_array($dtp->CTags))
        {
            foreach($dtp->CTags as $ctag)
            {
                ...
                {
                    $nForm .= " <dd><span class='dlleft'>软件地址".$newRowStart."</span><span class='dlright'><input type='text' name='softurl".$newRowStart."'  value='".trim($ctag->GetInnerText())."'  maxlength='100' class='linput' style='width:250px;margin-right:10px;'/> <input class='linput' type='text' name='servermsg".$newRowStart."' value='".$ctag->GetAtt("text")."' style='width:150px'/></span></dd>
