...
{
	$articles_flash .= "
	<li style='width: 614px; left: 0px; transition: 300ms; -webkit-transition: 300ms; -webkit-transform: translate(-614px, 0px) translateZ(0px);'><a href='wap.php?action=article&amp;id={$row->id}'>
			<img src='/wap/images/space.png' style='background-image: url({$row->litpic});' width='100%' height='196' alt=''>
			<p class='mask'>
				<em class='tit'>{$row->title}</em>
			</p>
			</a>
		</li>
";
...
else if($action=='article')
{
	//文档信息
	$query = "
	  Select tp.typename,tp.ishidden,arc.typeid,arc.title,arc.litpic,arc.arcrank,arc.pubdate,arc.writer,arc.click,addon.body From `#@__archives` arc 
