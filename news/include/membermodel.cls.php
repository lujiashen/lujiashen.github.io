                ...
                if($tag->GetAtt('autofield'))
                {
                    ...
                    公开所有联系方式</div></li>';
                            ...
                            } else if ($tag->GetAtt('type') == 'checkbox')
                            {
                             ...
                             if($tag->GetName() == 'onlynet')
                                {
                                    $formstring .= '<li style="display:none"><label>联系方式限制：</label>
                    <input name="onlynet" type="radio" id="onlynet" value="2" checked="checked" />
                    不公开所有联系方式
                    <input name="onlynet" type="radio" id="onlynet" value="1" />
                    不公开电话、详细地址
                    ...
                    公开所有联系方式</li>';
                                    ...
                                    $formtitle = ($tag->GetName() == 'place')? '目前所在地' : '家乡所在地';
                                    $formstring .='<li><span>'.$formtitle.'：</span><div class="vocation">' . GetEnumsForm('nativeplace',$value[$tag->GetName()],$tag->GetName()).'</div></li>';
                                } else if ($tag->GetName() == 'birthday'){
                                    $formstring .='<li><span>'.$tag->GetAtt('itemname').'：</span><input type="text" class="intxt" style="width: 100px;" id="birthday" value="'.$value[$tag->GetName()].'" name="birthday"></li>';
                                } else if (array_key_exists($tag->GetName(),$this->egroups)){
                                    //对联动模型进行特殊处理
                                    $formstring .='<li><span>'.$this->egroups[$tag->GetName()].'：</span> '. GetEnumsForm($tag->GetName(),$value[$tag->GetName()],$tag->GetName()).'</li>';
                                } else if ($tag->GetAtt('type') == 'checkbox'){
                                    //对checkbox模型进行特殊处理
                                  ...
                                  else if ($tag->GetAtt('type') == 'img')
                                {
                                    ...
                                    $imgstrng = "<li><label>{$labelname}：</label><input type='text' name='$fieldname' value='$fvalue' id='$fieldname' style='width:300px'  class='text' /> <input name='".$fieldname."_bt' class='inputbut' type='button' value='浏览...' onClick=\"SelectImage('addcontent.$fieldname','big')\" />\r\n</li>";
                                    $formstring .=$imgstrng;
